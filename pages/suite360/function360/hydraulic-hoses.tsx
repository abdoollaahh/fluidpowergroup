...
import componentData from '../../../data/function360/hydraulic-hoses.json';
...
export default function HydraulicHoses() {
  ...
  useEffect(() => {
    setIsSelected(config.selectedComponents.hydraulicHoses);  // ← Keep component key
  }, [config.selectedComponents.hydraulicHoses]);

  const handleSelect = () => {
    if (!isSelected) {
      toggleComponent('hydraulicHoses');  // ← Use correct key for each page
      setIsSelected(true);
      setIsSkipped(false);
    }
  };
