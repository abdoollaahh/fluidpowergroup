...
const SEARCHPARAMETERS = [
  {
    ...
    subCategories: ["MALE STRAIGHT", "FEMALE STRAIGHT", "45\u00B0", "90\u00B0", "BANJO"],  
  },
  {
...
const Search = () => {
  ...
  const addCategory = (product: string, category: string) => {
    ...
    if (found) {
      ...
      categories.push({ title: product, categories: [category], subCategories: [] });
    }
  ...
  const addSubCategory = (product: string, subCategory: string) => {
    ...
    if (found) {
      ...
      categories.push({ title: product, categories: [], subCategories: [subCategory] });
    }
    setCategories([...categories]);
  };
  ...
  useEffect(() => {
    ...
    categories.forEach((category) => {
      ...
      category.categories.forEach((cat) => {
        const foundCategory = found.subCategories.find((c: any) => c.title === cat);
  ...
  return (
    <div className="px-8 md:px-12   py-12 min-h-screen">
      <div className="text-[4rem] md:text-[6rem] lg:text-[8rem] xl:text-[10rem] font-bold text-slate-200/50 uppercase">
        Search
      </div>
      <div className="wrapper flex flex-col gap-12 items-center px-10">
        <div className="border-[1px] border-black w-full px-2 rounded-md">
          <div>
            <div>
              {
                SEARCHPARAMETERS.map((product, index) => (
                 <div key={index} className="flex flex-col md:flex-row justify-start items-start my-2">
                <div
                  className="w-[30%] max-w-[200px]"
                    onClick={() => {
                      if (titles.includes(product.product)) {
                        setTitles(titles.filter((title) => title !== product.product));
                      } else {
                ...
                >
                      <span
                        className={`text-lg font-bold ${titles.includes(product.product) ? "text-primary" : "text-slate-700"} cursor-pointer`}
                      >{product.product}</span>
                </div>
                    {titles.includes(product.product) &&
                      <div>
                        <div>
                          <span className="text-md font-bold text-black">Categories</span>
                          <div className="flex mt-2 flex-wrap">
                            {product.categories.map((category, index) => (
                              <div
                                className=
                                {
                                  `border-2 rounded-xl mr-4 px-4 py-1 flex justify-center items-center my-2 hover:cursor-pointer
                                  ${checkCategory(product.product, category) ? "border-primary text-primary" : "border-slate-700"}
                                  `
                                }
                                key={index}
                                onClick={() => {addCategory(product.product, category)} }
                                >
                                {category}
                              </div>
                            ))}
                        ...
                        <div className="mt-2">
                          <span className="text-md font-bold text-black">{product.subCategories.length > 0 && "Sub-Categories"}</span>
                            ...
                            {product.subCategories.map((subCategory, index) => (
                              <div 
                                className={`
                                  ...
                                  ${checkSubCategory(product.product, subCategory) ? "border-primary text-primary" : "border-slate-700"}
                                  `
                                }
                                key={index}
                                onClick={() => {addSubCategory(product.product, subCategory)} }
                              >
                                {subCategory}
                              </div>
                            ))}
                        ...
                        </div>
                        {
                          product.extraParams && (
                            <div className="mt-2">
                              <span className="text-md font-bold text-black">{ "Parameters"}</span>
                              <div className="flex mt-2 flex-wrap mb-5">
                                {product.extraParams.map((params, index) => (
                                  <div 
                                    ...
                                    ${checkSubCategory(product.product, params) ? "border-primary text-primary" : "border-slate-700"}`}
                                    key={index}
                                    onClick={() => {addSubCategory(product.product, params)} }
                                  >
                                    {params}
                                  </div>
                                ))}
      ...
      <div className="p-10">
        <GridProducts seriesList={filteredData} showDescription={Boolean(false)} />
      </div>
    </div>
